...
function testimonial_slider_get_textonly($slider_handle,$r_array,$testimonial_slider_curr,$set,$echo='1',$data=array()){
	...
	if ($testimonial_slider_curr['prev_next'] != 1){ 
	  $prevnext='next:   "#'.esc_attr($slider_handle).'_next", 
				 prev:   "#'.esc_attr($slider_handle).'_prev",';
	}
	$type='';
	...
	if ($testimonial_slider_curr['navnum'] == "1"){ 
		$nav_top='';
		$nav_bottom='<div id="'.esc_attr($slider_handle).'_nav" class="testimonial_nav  testimonial_nav-fillup" '.$testimonial_slider_css['testimonial_nav'].'></div>';
		$pagination='pagination  : { container: "#'.esc_attr($slider_handle).'_nav",
			anchorBuilder: function( nr ) {
				return \'<div class="inner_nav" '.$testimonial_slider_css['testimonial_nav_a'].'><a href="#" ></a></div>\';
			} },';
    } 
	if ($testimonial_slider_curr['navnum'] == "2"){ 
		$nav_top='<div id="'.esc_attr($slider_handle).'_nav" class="testimonial_nav testimonial_nav-fillup" '.$testimonial_slider_css['testimonial_nav'].'></div>';
		$nav_bottom='';
		$pagination='pagination  : { container: "#'.esc_attr($slider_handle).'_nav",
			anchorBuilder: function( nr ) {
				return \'<div class="inner_nav" '.$testimonial_slider_css['testimonial_nav_a'].'><a href="#" ></a></div>\';
	...
	$script.='jQuery(function() {
			jQuery("#'.esc_attr($slider_handle).'").testiMonial({
				'.esc_js($o_responsive).'
				items: 	{
					'.esc_js($o_width).'
	...
	<div id="'.esc_attr($slider_handle).'_wrap" class="testimonial_slider testimonial_slider_set'. $set .' testimonial_slider__'.$stylesheet.'" '.$testimonial_slider_css['testimonial_slider'].'>
		'.$sldr_title.$nav_top.'
		<div id="'.esc_attr($slider_handle).'" class="testimonial_slider_instance">
			'. $r_array[1] .'
		</div>
		'.$nav_bottom.'
		<div class="testimonial_nav_arrow_wrap">
			<a class="testimonial_prev" id="'.esc_attr($slider_handle).'_prev" href="#" '.$testimonial_slider_css['testimonial_prev'].'><span>prev</span></a>
			<a class="testimonial_next" id="'.esc_attr($slider_handle).'_next" href="#" '.$testimonial_slider_css['testimonial_next'].'><span>next</span></a>
		</div>
	</div>';
